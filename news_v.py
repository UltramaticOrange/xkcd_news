...
def image64(url, imgTag=False, maxSize=C.IMG_MAX_SIZE): # 'img' is abbriviated to reflect the html <img /> tag.
  ...
  try:
    ...
    logging.error(log_messages.E_UNABLE_TO_FETCH%(response.status_code, url))
    return None

  rawImage = response.content # get the image to be manipulated
  imageType = imghdr.what(StringIO(rawImage)) # returns an empty string if it can't be ID'd as a valid pic type.
  ...
  if imageObj.size[0] > maxSize or imageObj.size[1] > maxSize:
